...
- [ ] the following will have language-specific quirks that are currently not specified by pytorch as some attribute
  - python and julia differ in their flatten commands (one is row-major, the other column-major). For consistency, we only support tensors up to dimension 5, and their order is explicitly Width Height Depth Channel Batch
